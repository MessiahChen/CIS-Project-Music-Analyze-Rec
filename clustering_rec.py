...
def k_num_plot_librosa(X, min, max, step):
    ...
    plt.title('Optimal k for Librosa Features')
    plt.savefig('librosa_k_num_plot.png')
...
def k_num_plot_echonest(Y, min, max, step):
    ...
    plt.xlabel('k')
    plt.ylabel('Distortion')
    plt.title('Optimal k for Echonest Features')
    plt.savefig('echonest_k_num_plot.png')
...
# 3 songs in new genre, same cluster
